...
try {
  ...
  console.warn('⚠️  Brak serviceAccountKey.json — endpointy wymagające auth będą odrzucać requesty');
}
...
const ALLOWED_LAMP_COLS = new Set([
  'lat','lng','nr_slupa','rodzaj_slupa','liczba_opraw','kat_wysiegnika',
  'dlugosc_wysiegnika','rodzaj_oprawy','model_oprawy','stan_slupa','stan_oprawy',
  'wysokosc_slupa','szafa_oswietleniowa','rodzaj_linii','miejscowosc','ulica',
  'notes','usterka','usterka_typ','usterka_opis','moc_oprawa_w',
  'odleglosc_od_krawedzi','segment_id','photo',
]);
  ...
  try {
    await pool.query(`
      ...
      [d.id, d.lat, d.lng, d.nr_slupa, d.rodzaj_slupa, d.liczba_opraw, d.kat_wysiegnika,
       ...
       d.wysokosc_slupa, d.moc_oprawa_w||null, d.odleglosc_od_krawedzi||null,
       d.szafa_oswietleniowa, d.rodzaj_linii, d.miejscowosc, d.ulica, d.notes,
       d.usterka||'NIE', d.usterka_typ||'', d.usterka_opis||'', d.photo||null, d.segment_id||null]
    );
  ...
  try {
    await pool.query(`
      ...
      [id, d.lat, d.lng, d.nr_slupa, d.rodzaj_slupa, d.liczba_opraw, d.kat_wysiegnika,
       ...
       d.wysokosc_slupa, d.moc_oprawa_w||null, d.odleglosc_od_krawedzi||null,
       d.szafa_oswietleniowa, d.rodzaj_linii, d.miejscowosc, d.ulica, d.notes,
       d.usterka||'NIE', d.usterka_typ||'', d.usterka_opis||'']
    );
...
app.post('/api/segments', verifyToken, async (req, res) => {
  ...
  try {
    const r = await pool.query(`
      ...
      RETURNING id`,
      [d.nazwa, JSON.stringify(d.geojson), d.dlugosc_m,
       d.klasa_oswietleniowa, d.nawierzchnia, d.szerokosc_jezdni_m,
       d.liczba_pasow||2, d.predkosc_max_kmh||50, d.natezenie_ruchu,
       d.control_box_id||null, d.miejscowosc, d.ulica, d.notes]
    );
  ...
  try {
    await pool.query(`
      UPDATE road_segments SET
        ...
        updated_at=NOW()
      WHERE id=$1`,
      [req.params.id, d.nazwa, JSON.stringify(d.geojson), d.dlugosc_m,
       d.klasa_oswietleniowa, d.nawierzchnia, d.szerokosc_jezdni_m,
       d.liczba_pasow, d.predkosc_max_kmh, d.natezenie_ruchu, d.ulica, d.notes]
    );
    res.json({ ok: true });
  } catch (err) {
...
app.post('/api/boxes', verifyToken, async (req, res) => {
  ...
  try {
    const r = await pool.query(`
      ...
      RETURNING id`,
      [d.numer, d.lat, d.lng, d.miejscowosc, d.ulica,
       d.moc_przylacza||null, d.taryfa, d.licznik_nr, d.stan||'dobry', d.notes]
    );
    res.status(201).json({ id: r.rows[0].id });
  } catch (err) {
...
app.put('/api/boxes/:id', verifyToken, async (req, res) => {
  ...
  try {
    await pool.query(`
      ...
      WHERE id=$1`,
      [req.params.id, d.numer, d.miejscowosc, d.ulica,
       d.moc_przylacza||null, d.taryfa, d.licznik_nr, d.stan, d.notes]
    );
    res.json({ ok: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
...
app.post('/api/audits', verifyToken, async (req, res) => {
  ...
  try {
    const r = await pool.query(`
      ...
      [d.nazwa, d.miejscowosc, d.data_audytu||new Date().toISOString().slice(0,10),
       d.autor, d.cena_kwh||0.75, d.taryfa||'C11', d.godziny_pracy_rok||4000]
    );
    res.status(201).json({ id: r.rows[0].id });
  } catch (err) {
...
app.put('/api/audits/:id/results', verifyToken, async (req, res) => {
  ...
  try {
    await pool.query(`
      ...
      WHERE id=$1`,
      [req.params.id,
       d.moc_istniejaca_kw, d.moc_opcja1_kw, d.moc_opcja2_kw, d.moc_opcja3_kw,
       d.koszt_energii_rok_istniejacy,
       d.oszczednosc_opcja1_pln_rok, d.oszczednosc_opcja2_pln_rok, d.oszczednosc_opcja3_pln_rok,
       d.emisja_co2_istniejaca_kg_rok,
       d.koszt_inwestycji_opcja1, d.koszt_inwestycji_opcja2, d.koszt_inwestycji_opcja3,
